...
public class TemplateParser {
	...
	public static String analyzeTemplate(String code, List<Symbol> symbols) {
		...
		try {
			while (hasTemplateCode(symbols, analyticalCode.toString())) {
				...
				if (hasTemplateCode(symbols, templateCode)) {
				...
				for (Symbol oneSymbol : symbols) {
					if (oneSymbol.getSymbolStart().equals(templateBean.getStartSymbol())) {
						...
						analyticalCode.replace(templateBean.getStartIndex(), templateBean.getEndIndex()
								+ templateBean.getEndSymbol().length(), parsedCode);
					}
				}
			}
		} catch (Exception e) {
	...
	public static TemplateBean getFirstTemplateSymbol(String code, List<Symbol> symbols) {
		...
		// 判断代码中是否存在任意一种模板标记
		if (hasTemplateCode(symbols, code)) {
			// 找到模板中第一个出现的模板符号
	 ...
	 * @since 1.0
	 */
	public static boolean hasTemplateCode(List<Symbol> symbols, String code) {
		for (Symbol oneSymbol : symbols) {
	 ...
	 * @since 1.0
	 */
	public static TemplateBean getFirstTemplateBean(String string, int fromIndex, String start, String end) {
		int startIndex = string.indexOf(start, fromIndex);

		if (startIndex != -1) {
			int endIndex = getEndSymbolIndex(string, startIndex + start.length(), start, end);

			if (endIndex != -1) {
				TemplateBean templateBean = new TemplateBean();
				templateBean.setStartIndex(startIndex);
				templateBean.setEndIndex(endIndex);
				templateBean.setCode(string.substring(startIndex + start.length(), endIndex));
				templateBean.setStartSymbol(start);
				templateBean.setEndSymbol(end);
				return templateBean;
			} else {
				return null;
			}
